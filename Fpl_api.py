...
def FPLapi_entry_endpoint(ID):
    base_url = 'https://fantasy.premierleague.com/api/entry/'
    data = requests.get(base_url + str(ID) + '/').json()
    return data

def gameweek_picks(ID, gameweek = None, parse = False):
    data = FPLapi_entry_endpoint(ID)
    main_data = FPLapi_main_endpoint()
    if gameweek == None:
        gameweek = data['current_event']
    base_url = 'https://fantasy.premierleague.com/api/entry/'
    if parse == False:
        return requests.get(base_url + str(ID) + '/event/' + str(gameweek) + '/picks/').json()
    else:
        df = requests.get(base_url + str(ID) + '/event/' + str(gameweek) + '/picks/').json()
        for i in df['picks']:
            player = search_player_ID(i['element'])
            i['player'] = player
        return df

def get_value(ID, gameweek = None):
    data = FPLapi_entry_endpoint(ID)
    return (data['entry_history']['value']/10)
